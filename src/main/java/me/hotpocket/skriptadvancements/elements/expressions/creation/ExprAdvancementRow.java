...
public class ExprAdvancementRow extends SimpleExpression<Number> {
    ...
    protected @Nullable Number[] get(Event e) {
        return new Number[]{getParser().getCurrentSection(SecAdvancement.class).getAdvancement().displayBuilder.getY()};
    }
    ...
    public void change(Event e, @Nullable Object[] delta, Changer.ChangeMode mode) {
        assert delta[0] != null;
        getParser().getCurrentSection(SecAdvancement.class).getAdvancement().displayBuilder.setY(min(max(((Number) delta[0]).floatValue(), 0), 9999999));
    }
}
