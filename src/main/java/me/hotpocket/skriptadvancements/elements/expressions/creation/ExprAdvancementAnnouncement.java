...
public class ExprAdvancementAnnouncement extends SimpleExpression<Boolean> {
    ...
    protected @Nullable Boolean[] get(Event e) {
        return new Boolean[]{getParser().getCurrentSection(SecAdvancement.class).getAdvancement().displayBuilder.announceChat()};
    }
    ...
    public void change(Event e, @Nullable Object[] delta, Changer.ChangeMode mode) {
        assert delta[0] != null;
        getParser().getCurrentSection(SecAdvancement.class).getAdvancement().displayBuilder.setAnnounce((Boolean) delta[0]);
    }
}
